...
const xmlTemplate = [
  ...
  (result, current) =>
    Object.assign(result, {
      [_.camelCase(current)]: fs.readFileSync(
        path.join(__dirname, 'xml-templates', `${current}.xml`),
        {
          encoding: 'utf8',
        },
      ),
...
IbmConnectionsProfilesService.prototype.getServiceDocument = function getServiceDocument(
  query,
  options,
  callback,
) {
  ...
  if (_.size(entrySelector) > 1) {
    const error = new Error(
      util.format(
        'Wrong number of entry selectors provided to get editable fields: %j',
        entrySelector,
      ),
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (
      ...
      !/application\/atomsvc\+xml/.test(response.headers['content-type'])
    ) {
      // eslint-disable-line max-len
      callback(new Error('received invalid response'));
      return;
    }
    callback(null, responseParser.serviceDocument(body));
  });
};

IbmConnectionsProfilesService.prototype.getProfileEntry = function getProfileEntry(
  query,
  options,
  callback,
) {
  ...
  const qsValidParameters = ['email', 'key', 'userid'];
  ...
  if (_.size(entrySelector) !== 1) {
    const error = new Error(
      util.format(
        'Wrong number of entry selectors provided to receive profile entry: %j',
        entrySelector,
      ),
    ); // eslint-disable-line max-len
    error.status = 400;
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (!response || statusCode !== 200) {
      const error = new Error(body || 'received invalid response');
    ...
    if (!(response.headers && contentType.startsWith('application/atom+xml'))) {
      const error = new Error(
        `received response with unexpected content-type ${contentType}`,
      );
    ...
    if (!body.includes(requestOptions.qs.output)) {
      const error = new Error(
        `Expected output type [${requestOptions.qs.output}] was not found in entry document. Please provide valid profile credentials`,
      ); // eslint-disable-line max-len
...
IbmConnectionsProfilesService.prototype.updateEntry = function updateEntry(
  entry,
  options,
  callback,
) {
  ...
  if (!entry || !entry.userid) {
    error = new Error(
      util.format(
        'A valid profile entry must be provided and have a "userid" property %j',
        entry,
      ),
    );
    error.status = 400;
  ...
  return self.getServiceDocument(
    ...
    (err, serviceDocument) => {
      ...
      const { editableFields } = serviceDocument;

      if (
        editableFields.indexOf('jobResp') > -1 &&
        entry.jobResp &&
        entry.jobResp.length > 128
      ) {
      ...
      const requestOptions = _.merge(omitDefaultRequestParams(options), {
        ...
        body: util.format(
          xmlTemplate.entry,
          self.vCardParser.toVcard(entry, editableFields),
        ),
        headers: {
      ...
      httpClient.makeRequest(requestOptions, (error, response) => {
        ...
        async.each(
          entry.extattr.filter(
            extattr =>
              editableFields.indexOf(extattr.id) > -1 &&
              serviceDocument.extattrDetails[extattr.id],
          ), // eslint-disable-line max-len
          (extattr, iteratorCallback) => {
            const extattrDetails = serviceDocument.extattrDetails[extattr.id];
            const extAttrRequestOptions = omitDefaultRequestParams(
              requestOptions,
              ['body'],
            );

            extAttrRequestOptions.uri = extattrDetails.href
              .split(self.requestOptions.baseUrl)
              .pop();
...
IbmConnectionsProfilesService.prototype.batchLoadEntries = function batchLoadEntries(
  entries,
  options,
) {
  const self = this;
  ...
  entries.forEach(entry => {
    self.getProfileEntry(
      _.pick(entry, ['userid', 'key', 'email']),
      options,
      _.noop,
    );
  });
...
IbmConnectionsProfilesService.prototype.getEditableFields = function getEditableFields(
  query,
  options,
  callback,
) {
...
IbmConnectionsProfilesService.prototype.getNetworkConnections = function getNetworkConnections(
  query,
  options,
  callback,
) {
  ...
  if (_.size(entrySelector) !== 1) {
    const error = new Error(
      util.format(
        'Wrong number of entry selectors provided to receive network connections: %j',
        entrySelector,
      ),
    );
    error.status = 400;
  ...
  if (
    !_.isString(requestOptions.qs.outputType) ||
    requestOptions.qs.outputType !== 'profile'
  ) {
  ...
  if (
    !_.isString(requestOptions.qs.output) ||
    ['hcard', 'vcard'].indexOf(requestOptions.qs.output) < 0
  ) {
    requestOptions.qs.output = 'vcard';
  }
  // format: 'lite',  // 'full'  this parameter is ignored if "outputType" is not set to "profile"
  if (
    !_.isString(requestOptions.qs.format) ||
    ['lite', 'full'].indexOf(requestOptions.qs.format) < 0
  ) {
    requestOptions.qs.format = 'lite';
  }
  if (
    !_.isString(requestOptions.qs.sortBy) ||
    ['displayName', 'modified'].indexOf(requestOptions.qs.sortBy) < 0
  ) {
    requestOptions.qs.sortBy = 'displayName';
  }
  if (
    !_.isString(requestOptions.qs.sortOrder) ||
    ['asc', 'desc'].indexOf(requestOptions.qs.sortOrder) < 0
  ) {
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    for (
      let i = 2;
      i <= Math.ceil(data.totalResults / requestOptions.qs.ps);
      i += 1
    ) {
      pageRequests.push(
        _.merge(_.clone(requestOptions), {
          qs: {
            page: i,
          },
        }),
    ...
    async.map(
      pageRequests,
      (pageRequestOptions, iteratorCallback) => {
      ...
      (err, results) => {
        ...
        batchLoadEntries(
          _.keys(result.networkConnections).map(userid => ({
            userid,
          })),
          options,
        );
...
IbmConnectionsProfilesService.prototype.getNetworkState = function getNetworkState(
  query,
  options,
  callback,
) {
  ...
  if (_.size(targetSelector) !== 1) {
    const error = new Error(
      util.format(
        'Wrong number of targetEntry selectors provided to receive network state: %j',
        targetSelector,
      ),
    ); // eslint-disable-line max-len
    error.status = 400;
  ...
  if (_.size(sourceSelector) !== 1) {
    const error = new Error(
      util.format(
        'Wrong number of sourceEntry selectors provided to receive network state: %j',
        sourceSelector,
      ),
    ); // eslint-disable-line max-len
    error.status = 400;
  ...
  httpClient.makeRequest(requestOptions, (err, response) => {
    ...
    if (
      response.headers[networkStatusHeaderName] &&
      ['accepted', 'pending', 'unconfirmed'].indexOf(
        response.headers[networkStatusHeaderName],
      ) > -1
    ) {
...
IbmConnectionsProfilesService.prototype.inviteNetworkContact = function inviteNetworkContact(
  query,
  options,
  callback,
) {
  ...
  const qsValidParameters = ['userid'];
  ...
  httpClient.makeRequest(requestOptions, (err, response) => {
    ...
    if (response.statusCode === 400) {
      ...
      const error = new Error('A pending invitation exists already');
...
IbmConnectionsProfilesService.prototype.removeFromNetwork = function removeFromNetwork(
  query,
  options,
  callback,
) {
...
IbmConnectionsProfilesService.prototype.getFollowedProfiles = function getFollowedProfiles(
  query,
  options,
  callback,
) {
  const { httpClient, omitDefaultRequestParams, batchLoadEntries } = this;
  const qsValidParameters = ['page', 'ps', 'resource'];
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    async.map(
      ...
      (err, results) => {
        ...
        // start prefetching all involved profile entries
        batchLoadEntries(
          _.keys(result.followedProfiles).map(userid => ({
            userid,
          })),
          options,
        );
...
IbmConnectionsProfilesService.prototype.getFollowState = function getFollowState(
  userid,
  options,
  callback,
) {
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (resourceId) {
      const validator = new RegExp(
        `urn:lsid:ibm.com:follow:resource-${userid}`,
        'i',
      );
...
IbmConnectionsProfilesService.prototype.follow = function follow(
  query,
  options,
  callback,
) {
  ...
  if (!query.targetUserid) {
    const error = new Error('entry.targetUserid must be provided');
    error.httpStatus = 400;
    callback(error);
    return;
  }
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (resourceId) {
      const validator = new RegExp(
        `urn:lsid:ibm.com:follow:resource-${query.targetUserid}`,
        'i',
      );
      callback(null, validator.test(resourceId));
      return;
...
IbmConnectionsProfilesService.prototype.unfollow = function unfollow(
  query,
  options,
  callback,
) {
  ...
  if (!query.targetUserid) {
    const error = new Error('query.targetUserid must be provided');
    error.httpStatus = 400;
    callback(error);
    return;
  }
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    const editLink = xpathSelect(
      '/atom:entry/atom:link[rel="edit"]',
      data,
      true,
    );
    if (!(editLink && editLink.getAttribute('href'))) {
      const error = new Error(
        `Can not find followed resource: ${query.targetUserid}`,
      );
      error.httpStatus = 400;
      callback(error);
      return;
...
IbmConnectionsProfilesService.prototype.getTags = function getTags(
  query,
  options,
  callback,
) {
  ...
  const targetSelector = _.pick(requestOptions.qs, [
    'targetUserid',
    'targetEmail',
    'targetKey',
  ]);
  if (_.size(targetSelector) !== 1) {
    const error = new Error(
      util.format(
        'Wrong number of targetEntry selectors provided to receive tags: %j',
        targetSelector,
      ),
    ); // eslint-disable-line max-len
    error.status = 400;
  ...
  const sourceSelector = _.pick(requestOptions.qs, [
    'sourceUserid',
    'sourceEmail',
    'sourceKey',
  ]);
  if (_.size(sourceSelector) > 1) {
    const error = new Error(
      util.format(
        'Wrong number of sourceEntry selectors provided to receive tags: %j',
        sourceSelector,
      ),
    ); // eslint-disable-line max-len
    error.status = 400;
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (data.contributors) {
      batchLoadEntries(
        Object.keys(data.contributors).map(userid => ({
          userid,
        })),
        options,
      );
...
IbmConnectionsProfilesService.prototype.updateTags = function updateTags(
  query,
  tags,
  options,
  callback,
  ...
  if (!Array.isArray(tags)) {
    const error = new Error('an Array of tags must be provided');
    error.status = 400;
    callback(error);
    return;
  }
  ...
  if (_.size(targetSelector) !== 1) {
    const error = new Error(
      util.format(
        'Wrong number of targetEntry selectors provided to update tags: %j',
        targetSelector,
      ),
    ); // eslint-disable-line max-len
    error.status = 400;
  ...
  if (_.size(sourceSelector) > 1) {
    const error = new Error(
      util.format(
        'Wrong number of sourceEntry selectors provided to update tags: %j',
        sourceSelector,
      ),
    ); // eslint-disable-line max-len
    error.status = 400;
  ...
  const tagsDocumentParentNode = xpathSelect(
    'app:categories',
    tagsDocument,
    true,
  );
  ...
  httpClient.makeRequest(requestOptions, (err, response) => {
    ...
    // content-type: application/atom+xml
    if (!response || response.statusCode !== 200) {
      const error = new Error('received invalid response');
      error.httpStatus = response.statausCode;
      callback(error);
      return;
    }
...
IbmConnectionsProfilesService.prototype.addTags = function addTags(
  query,
  tags,
  options,
  callback,
  ...
  const sourceSelector = _.pick(query, [
    'sourceUserid',
    'sourceEmail',
    'sourceKey',
  ]);
...
IbmConnectionsProfilesService.prototype.removeTags = function removeTags(
  query,
  tags,
  options,
  callback,
  ...
  return getTags(query, options, (err, existingTags) => {
    ...
    // --> easier to compare
    const oldTags = existingTags.tags.map(tag =>
      JSON.stringify(_.pick(tag, 'term', 'type')),
    );
    ...
    return updateTags(
      query,
      _.difference(oldTags, tagsToRemove).map(tag => JSON.parse(tag)),
      options,
      callback,
    );
  });
