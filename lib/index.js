...
IbmConnectionsProfilesService.prototype.getNetworkConnections = function getNetworkConnections(query, options, callback) { // eslint-disable-line max-len
  ...
  const entrySelector = _.pick(requestOptions.qs, 'email', 'key', 'userid');
  if (_.size(entrySelector) !== 1) {
    const error = new Error(util.format('Wrong number of entry selectors provided to receive network connections: %j',
      entrySelector));
    error.status = 400;
    callback(error);
    return;
...
IbmConnectionsProfilesService.prototype.getNetworkState = function getNetworkState(query, options, callback) {
  ...
  if (_.size(targetSelector) !== 1) {
    const error = new Error(util.format('Wrong number of targetEntry selectors provided to receive network state: %j',
      targetSelector));
    error.status = 400;
    callback(error);
    return;
  ...
  if (_.size(sourceSelector) !== 1) {
    const error = new Error(util.format('Wrong number of sourceEntry selectors provided to receive network state: %j',
      sourceSelector));
    error.status = 400;
    callback(error);
    return;
...
IbmConnectionsProfilesService.prototype.getTags = function getTags(query, options, callback) {
  ...
  const targetSelector = _.pick(requestOptions.qs, ['targetUserid', 'targetEmail', 'targetKey']);
  if (_.size(targetSelector) !== 1) {
    const error = new Error(util.format('Wrong number of targetEntry selectors provided to receive tags: %j',
      targetSelector));
    error.status = 400;
    callback(error);
    return;
  ...
  const sourceSelector = _.pick(requestOptions.qs, ['sourceUserid', 'sourceEmail', 'sourceKey']);
  if (_.size(sourceSelector) > 1) {
    const error = new Error(util.format('Wrong number of sourceEntry selectors provided to receive tags: %j',
      sourceSelector));
    error.status = 400;
    callback(error);
    return;
...
IbmConnectionsProfilesService.prototype.updateTags = function updateTags(query, tags, options, callback) {
  ...
  if (_.size(targetSelector) !== 1) {
    const error = new Error(
      util.format('Wrong number of targetEntry selectors provided to update tags: %j', targetSelector)
    );
    error.status = 400;
    callback(error);
    return;
  ...
  if (_.size(sourceSelector) > 1) {
    const error = new Error(
      util.format('Wrong number of sourceEntry selectors provided to update tags: %j', sourceSelector)
    );
    error.status = 400;
    callback(error);
    return;
