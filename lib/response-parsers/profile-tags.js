...
const addValToNestedArray = (obj, path, val) => _.set(obj, path, _.union(_.get(obj, path, []), [val]));

const addTagTermToContributions = (contributor, tag) =>
  addValToNestedArray(contributor, `contributions.${tag.type}`, tag.term);

const addContributorToTag = (contributor, tag) => addValToNestedArray(tag, 'contributors', contributor.userid);
