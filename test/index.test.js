...
const {
  unmocked, username, password, albert_userid,
} = process.env;
  ...
  serviceInstance.getProfileEntry(
    ...
    (err, result) => {
      ...
      ['surname', 'givenName'].forEach(prop =>
        t.true(prop in names, `${prop} should be a member of {{ names }} object`));
      t.end();
    }
  );
});
    ...
    (err, result) => {
      ...
      _.keys(networkConnections).forEach((colleagueKey) => {
        ...
        ['type', 'connectionType', 'status'].forEach(category =>
          t.true(category in categories, `[${category}] should be a member of categories object`));
  ...
  serviceInstance.getServiceDocument(
    ...
    (err, result) => {
      ...
      editableFields.forEach(field =>
        t.true(editableFieldsProps.includes(field), `[${field}] should be a member of editableFields array`));
      ...
      linksProps.forEach((link) => {
        ...
        linkItemProps.forEach(linkItemProp =>
          t.true(linkItemProp in linkItem, `[${linkItemProp}] should be a member of linkItem object`));
      });
...
test.cb('error validation for retrieving profile entry using Profile service instance, userid not provided', (t) => {
  const { serviceInstance } = t.context;
