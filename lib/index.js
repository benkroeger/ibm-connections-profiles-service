...
IbmConnectionsProfilesService.prototype.getServiceDocument = function getServiceDocument(query, options, callback) {
  ...
  if (_.size(entrySelector) > 1) {
    const error = new Error(util.format('Wrong number of entry selectors provided to get editable fields: %j', entrySelector));
    error.status = 400;
    callback(error);
    return;
  }
...
IbmConnectionsProfilesService.prototype.getProfileEntry = function getProfileEntry(query, options, callback) {
  ...
  if (_.size(entrySelector) !== 1) {
    const error = new Error(util.format('Wrong number of entry selectors provided to receive profile entry: %j', entrySelector));
    error.status = 400;
    callback(error);
    return;
  }
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (!body.includes(requestOptions.qs.output)) {
      const error = new Error(`Expected output type [${requestOptions.qs
        .output}] was not found in entry document. Please provide valid profile credentials`); // eslint-disable-line max-len
...
IbmConnectionsProfilesService.prototype.getNetworkState = function getNetworkState(query, options, callback) {
  ...
  if (_.size(targetSelector) !== 1) {
    const error = new Error(util.format('Wrong number of targetEntry selectors provided to receive network state: %j', targetSelector));
    error.status = 400;
    callback(error);
    return;
  }
  ...
  if (_.size(sourceSelector) !== 1) {
    const error = new Error(util.format('Wrong number of sourceEntry selectors provided to receive network state: %j', sourceSelector));
    error.status = 400;
    callback(error);
    return;
  }
