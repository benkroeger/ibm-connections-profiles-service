...
const parseCategories = nodes => _.reduce(nodes, (result, node) => {
  const category = parseXMLNode(node, {
...
const parseFollowedProfilesEntries = (responseXML) => {
  ...
  const followedProfiles = _.reduce(entryNodes, (result, entryNode) => {
    const profile = parseXMLNode(entryNode, {
      id: {
