...
const findOrCreateContributor = (catalog, xmlNode) => {
  ...
  Object.assign(contributor, {
    ...
    email: xpathSelect('string(atom:email/text())', xmlNode, true),
    userState: xpathSelect('string(snx:userState/text())', xmlNode, true),
    isExternal: xpathSelect('string(snx:isExternal/text())', xmlNode, true),
  });
...
module.exports = stringOrXMLDoc => {
  ...
  const [categoriesTagNode] = xmlDoc.getElementsByTagNameNS(
    xmlNS.app,
    'categories',
  );
  const categoryTagNodes = categoriesTagNode.getElementsByTagNameNS(
    xmlNS.atom,
    'category',
  );
  const numberOfContributors = parseInt(
    categoriesTagNode.getAttributeNS(xmlNS.snx, 'numberOfContributors'),
    10,
  );

  const contributors = {};
  const tags = _.map(categoryTagNodes, categoryTagNode => {
    const contributorTagNodes = categoryTagNode.getElementsByTagNameNS(
      xmlNS.atom,
      'contributor',
    );
    ...
    _.forEach(contributorTagNodes, contributorTagNode => {
      const contributor = findOrCreateContributor(
        contributors,
        contributorTagNode,
      );
