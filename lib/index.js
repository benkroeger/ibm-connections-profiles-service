...
const xmlTemplate = ['entry', 'follow-entry', 'make-friend', 'tags-document']
  .reduce(function (result, current) {
    result[_.camelCase(current)] = fs.readFileSync(path.join(__dirname, 'xml-templates', current + '.xml'), {
      encoding: 'utf8'
    });
    return result;
  }, {});
...
const responseParser = {
  profileService: function parseProfileServiceResponse(responseXML) {
    ...
    const result = {
      ...
      editableFields: Array.prototype.map.call(selectXPath('/app:service/app:workspace/app:collection/snx:editableFields/snx:editableField', responseXML), function (element) {
    ...
    Array.prototype.forEach.call(selectXPath('/app:service/app:workspace/atom:link[@rel]', responseXML), function (element) {
  ...
  profileEntry: function parseProfileEntryResponse(responseXML, parser, isEntryNode) {
    ...
    const vcardString = selectXPath(xpathExpression, responseXML, true).toString();
    if (vcardString === '') {
  ...
  followedProfiles: function parseFollowedProfilesResponse(responseXML) {
    ...
    Array.prototype.forEach.call(responseXML.getElementsByTagName('entry'), function (followedEntry) {
      const followedResourceId = followedEntry.getElementsByTagName('id')[0].textContent.split('urn:lsid:ibm.com:follow:resource-')[1];
...
function IbmConnectionsProfilesService(baseUrl, options) {
  options = _.merge({
    ...
    vCardParser: {
      vCardToJSONAttributeMapping: require('./vcard-attribute-mapping.json'),
  ...
  httpClient.use(credentialsPlugins(options.plugins.credentials));

  this.vCardParser = new OniyiVCardParser(options.vCardParser);
  this._options = options;
  this.httpClient = httpClient;

  this.extractRequestParams = function (params) {
    let omit = ['baseUrl', 'uri', 'url', 'method', 'qs'];
    return _.omit(params, omit);
  };
}

IbmConnectionsProfilesService.prototype.getServiceDocument = function getServiceDocument(query, options, callback) {
  const { httpClient, extractRequestParams } = this;
  let error;
  ...
  const requestOptions = _.merge(extractRequestParams(options), {
  ...
  requestOptions.uri = 'atom/profileService.do';
  ...
  httpClient.makeRequest(requestOptions, function (err, response, body) {
    if (err) {
      return callback(err);
    }
    // expexted
...
IbmConnectionsProfilesService.prototype.getEntry = function getEntry(query, options, callback) {
  const { httpClient, extractRequestParams, vCardParser, _options } = this;
  ...
  const requestOptions = _.merge({
    ...
    ttl: _options.ttl.profileEntry
  });

  requestOptions.uri = 'atom/profileEntry.do';
...
IbmConnectionsProfilesService.prototype.updateEntry = function updateEntry(entry, options, callback) {
  ...
  return self.getServiceDocument({
    ...
    // construct the request options
    const requestOptions = _.merge(self.extractRequestParams(options), {
      method: 'PUT',
      qs: {
        userid: entry.userid
      },
    ...
    self.makeRequest(requestOptions, function (err, response) {
      ...
      async.each(
        ...
        function (extattr, iteratorCallback) {
          ...
          const extAttrRequestOptions = self.extractRequestParams(requestOptions, ['body']);
...
IbmConnectionsProfilesService.prototype.getNetworkConnections = function getNetworkConnections(query, options, callback) {
  ...
  self.makeRequest(requestOptions, function (err, response, body) {
    ...
    const data = responseParser.networkConnections(body, self.vCardParser);
    ...
    for (let i = 2; i <= Math.ceil(data.totalResults / requestOptions.qs.ps); i++) {
  ...
  self.makeRequest(requestOptions, function (err, response, body) {
    ...
    async.map(pageRequests, function (pageRequestOptions, iteratorCallback) {
      self.makeRequest(pageRequestOptions, function (err, response, body) {
        ...
        const data = responseParser.followedProfiles(body); // , self.vCardParser);
        iteratorCallback(null, data);
      });
    }, function (err, results) {
      if (err) {
...
IbmConnectionsProfilesService.prototype.addTags = function addTags(query, tags, options, callback) {
  ...
  return self.getTags(query, options, function (err, existingTags) {
    ...
    // normalize old tags
    const oldTags = existingTags.tags.map(function (tag) {
...
IbmConnectionsProfilesService.prototype.removeTags = function removeTags(query, tags, options, callback) {
  ...
  return self.getTags(query, options, function (err, existingTags) {
    ...
    // normalize new tags
    const removeTags = tags.map(function (tag) {
      if (_.isString(tag)) {
        tag = {
          term: tag
        };
