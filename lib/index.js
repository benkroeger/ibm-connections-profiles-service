...
const responseParser = {
  profileService: require('./response-parsers/service-document'),
  profileEntry: function parseProfileEntryResponse(responseXML, parser, isEntryNode) {
    ...
    const vcardString = safeConvertToString(xpathSelect(xpathExpression, responseXML, true));
    if (!vcardString) {
  ...
  networkConnections: function parseNetworkConnectionsResponse(responseXML, vCardParser) {
    ...
    const paginationLinkElements = xpathSelect('/atom:feed/atom:link[@rel]', responseXML);
    ...
    if (_.isString(returnValue.paginationLinks.self) && /outputType=profile/i.test(returnValue.paginationLinks.self)) {
      Array.prototype.forEach.call(xpathSelect('/atom:feed/atom:entry', responseXML), (entryXML) => {
      ...
      _.forEach(xpathSelect('/atom:feed/atom:entry', responseXML), (entry) => {
        // could also detect who initialized the connection (author vs. contributor)
        const connection = {
          id: xpathSelect('atom:id/text()', entry, true).toString().split('tag:profiles.ibm.com,2006:entry').pop(),
          type: xpathSelect('atom:category[@scheme="http://www.ibm.com/xmlns/prod/sn/type"]/@term', entry, true).value,
          connectionType: xpathSelect('atom:category[@scheme="http://www.ibm.com/xmlns/prod/sn/connection/type"]/@term', entry, true).value,
          status: xpathSelect('atom:category[@scheme="http://www.ibm.com/xmlns/prod/sn/status"]/@term', entry, true).value,
          updated: safeConvertToString(xpathSelect('atom:updated/text()', entry, true)),
          message: safeConvertToString(xpathSelect('atom:content/text()', entry, true)),
          summary: safeConvertToString(xpathSelect('atom:summary/text()', entry, true)),
          links: {
            self: {
              href: xpathSelect('atom:link[@rel="self"]/@href', entry, true).value,
              type: xpathSelect('atom:link[@rel="self"]/@type', entry, true).value,
            },
            edit: {
              href: xpathSelect('atom:link[@rel="edit"]/@href', entry, true).value,
              type: xpathSelect('atom:link[@rel="edit"]/@type', entry, true).value,
            },
          },
        };
  ...
  followedProfiles: function parseFollowedProfilesResponse(responseXML) {
    ...
    Array.prototype.forEach.call(responseXML.getElementsByTagName('entry'), (followedEntry) => {
      ...
      const userid = xpathSelect('category[scheme="http://www.ibm.com/xmlns/prod/sn/resource-id"]/@term', followedEntry).value;
  ...
  profileTags: function parseProfileTagsResponse(responseXML) {
    ...
    Array.prototype.forEach.call(categoryTags, (categoryTag) => {
      ...
      Array.prototype.forEach.call(contributorTags, (contributorTag) => {
        ...
        const contributor = returnValue.contributors[contributorGuid] || { contribution: {} };
