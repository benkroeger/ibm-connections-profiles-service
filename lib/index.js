...
const xmlTemplate = ['entry', 'follow-entry', 'make-friend', 'tags-document'].reduce(
  (result, current) =>
    Object.assign(result, {
      [_.camelCase(current)]: fs.readFileSync(path.join(__dirname, 'xml-templates', `${current}.xml`), {
        encoding: 'utf8',
      }),
    }),
  {}
...
IbmConnectionsProfilesService.prototype.getServiceDocument = function getServiceDocument(query, options, callback) {
  ...
  if (_.size(entrySelector) > 1) {
    const error = new Error(util.format('Wrong number of entry selectors provided to get editable fields: %j', entrySelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
...
IbmConnectionsProfilesService.prototype.getProfileEntry = function getProfileEntry(query, options, callback) {
  ...
  if (_.size(entrySelector) !== 1) {
    const error = new Error(util.format('Wrong number of entry selectors provided to receive profile entry: %j', entrySelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (!body.includes(requestOptions.qs.output)) {
      const error = new Error(`Expected output type [${requestOptions.qs.output}] was not found in entry document. Please provide valid profile credentials`); // eslint-disable-line max-len
...
IbmConnectionsProfilesService.prototype.getNetworkConnections = function getNetworkConnections(
  ...
  // eslint-disable-line max-len
  const {
    httpClient, omitDefaultRequestParams, vCardParser, batchLoadEntries,
  } = this;
  ...
  const entrySelector = _.pick(requestOptions.qs, 'email', 'key', 'userid');
  if (_.size(entrySelector) !== 1) {
    const error = new Error(util.format('Wrong number of entry selectors provided to receive network connections: %j', entrySelector));
    error.status = 400;
    callback(error);
    return;
  }
...
IbmConnectionsProfilesService.prototype.getNetworkState = function getNetworkState(query, options, callback) {
  ...
  const qsValidParameters = ['targetUserid', 'targetEmail', 'targetKey', 'sourceUserid', 'sourceEmail', 'sourceKey'];
  ...
  if (_.size(targetSelector) !== 1) {
    const error = new Error(util.format('Wrong number of targetEntry selectors provided to receive network state: %j', targetSelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
  ...
  if (_.size(sourceSelector) !== 1) {
    const error = new Error(util.format('Wrong number of sourceEntry selectors provided to receive network state: %j', sourceSelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
  ...
  httpClient.makeRequest(requestOptions, (err, response) => {
    ...
    if (
      ...
      ['accepted', 'pending', 'unconfirmed'].indexOf(response.headers[networkStatusHeaderName]) > -1
    ) {
...
IbmConnectionsProfilesService.prototype.unfollow = function unfollow(query, options, callback) {
  ...
  httpClient.makeRequest(requestOptions, (err, response, body) => {
    ...
    if (!(editLink && editLink.getAttribute('href'))) {
      const error = new Error(`Can not find followed resource: ${query.targetUserid}`);
      error.httpStatus = 400;
      callback(error);
      return;
    }
...
IbmConnectionsProfilesService.prototype.getTags = function getTags(query, options, callback) {
  ...
  const targetSelector = _.pick(requestOptions.qs, ['targetUserid', 'targetEmail', 'targetKey']);
  if (_.size(targetSelector) !== 1) {
    const error = new Error(util.format('Wrong number of targetEntry selectors provided to receive tags: %j', targetSelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
  const sourceSelector = _.pick(requestOptions.qs, ['sourceUserid', 'sourceEmail', 'sourceKey']);
  if (_.size(sourceSelector) > 1) {
    const error = new Error(util.format('Wrong number of sourceEntry selectors provided to receive tags: %j', sourceSelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
...
IbmConnectionsProfilesService.prototype.updateTags = function updateTags(query, tags, options, callback) {
  ...
  const targetSelector = _.pick(requestOptions.qs, ['targetUserid', 'targetEmail', 'targetKey']);
  if (_.size(targetSelector) !== 1) {
    const error = new Error(util.format('Wrong number of targetEntry selectors provided to update tags: %j', targetSelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
  const sourceSelector = _.pick(requestOptions.qs, ['sourceUserid', 'sourceEmail', 'sourceKey']);
  if (_.size(sourceSelector) > 1) {
    const error = new Error(util.format('Wrong number of sourceEntry selectors provided to update tags: %j', sourceSelector)); // eslint-disable-line max-len
    error.status = 400;
    callback(error);
    return;
  }
